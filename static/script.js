document.addEventListener("DOMContentLoaded", () => {
  ...
  const menuResponses = {
    models: {
      message:
        "Here are our current Porsche models. Click on any model to know more:",
      options: [
    ...
    dealerships: {
      message:
        " Please enter nearest City/ZipCode or You can find your nearest Porsche dealership at below link:",
      options: [{ name: "Dealer Locator", link: PORSCHE_LINKS.dealership }],
    },
    "test-drive": {
      message: "Please visit below link for test drive and Experience Porsche:",
      options: [
        { name: "Schedule a Test Drive", link: PORSCHE_LINKS.testDrive },
        {
          name: "Porsche Experience Center",
          link: "https://www.porsche.com/usa/events-and-experiences/porsche-experience-center/",
        },
      ],
    },
    build: {
      message: "Please Visit Below Link to Build your dream Porsche:",
      options: [{ name: "Porsche Configurator", link: PORSCHE_LINKS.build }],
    },
  };
  ...
  function addMessage(message, isUser = false) {
    ...
    messageDiv.className = `flex items-start message ${
      isUser ? "justify-end" : ""
    }`;
    ...
    messageDiv.innerHTML = `
            ...
            <div class="${
              isUser ? "mr-3" : "ml-3"
            } ${bgColor} rounded-lg p-3 max-w-[70%]">
  ...
  function addSuggestedQuestions(questions) {
    ...
    questionsDiv.className = "flex flex-col space-y-2 mt-2 ml-12";

    questions.forEach((question) => {
      const button = document.createElement("button");
      button.className =
        "text-left p-2 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors text-sm text-blue-600";
  ...
  function addInteractiveOptions(options) {
    ...
    options.forEach((option) => {
      const button = document.createElement("button");
      button.className =
        "text-left p-2 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors";
      button.innerHTML = `
      ...
      button.onclick = () => {
        ...
        if (currentMenuOption === "models") {
          ...
          addMessage(
            `You selected the ${option.name}. What would you like to know about it?`
          );
        }
      };
  ...
  socket.on("response", (response) => {
    ...
    if (typeof response === "object") {
      ...
      if (response.suggested_questions) {
        addSuggestedQuestions(response.suggested_questions);
      }
    } else {
      addMessage(response);
    }
